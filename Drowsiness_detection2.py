...
Mouth_Radio_th = 0.5
Low_radio_constant = 3  # 意味着连续多少帧横纵比小于Radio小于阈值时，判断疲劳
...
def drowsinessDetection(Radio,cap):
    global alarm, frame_counter, mouth_alarm, mouth_frame_counter, head_alarm, head_frame_counter
    ...
    return frame, alarm, mouth_alarm, head_alarm
...
def main():
    ...
    while cap.isOpened():
        ...
        if count < 30:
            ...
            Radio = RadioSum/(1.0*30)-0.02
            start = True
            print('Radio = ',Radio)
        if start:
            frame = drowsinessDetection(Radio,cap)[0]
